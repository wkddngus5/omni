...
/**
 * 普通短信发送测试Main/test for common message sent
 * @author 
 *
 */
public class ChuanglanSMSTest {
	
	private static String account=""; // 发送短信的账号(非登录账号) (示例:N987654)/account sending message (not the login account)(example:N987654) 
	private static String pswd="";// 发送短信的密码(非登录密码)/password sending message (not the login password)
	...
	public static void sendMessage(){
		...
		String msg = "【欧米智能】测试短信,您的验证码是123456/[OMNI intelligent] test message, your verified code is 123456"; //您的签名+短信内容 / //your signature + message content
		...
		try {
			    String returnString2 = ChuanglanSMS.SendPost(url,"", "",msg.toString(),phone,rd,extno); 
	...
	public static void sendDelay() throws UnsupportedEncodingException {
		...
		String msg = "【253云通讯】测试定时短信内容/[253 yuntongxun] timed message test content";            // 短信内容/message content
		String rd="1";					// 是否需要状态报告(需要:1,不需要:0)/need the status report or not (yes:1, No, 0)
		String ex = null;     			// 扩展码(可选参数,可自定义)extended code(parameter optional, customizable)
		...
		try {
			String returnString = ChuanglanSMS.senddelay(url, account, pswd, phone, msg, rd, ex, delay);
	...
	public static void SendVariable() {
		...
		String extno = null;	  // 扩展码(可选参数,可自定义)/extended code(parameter optional, customizable)

		try {
			String returnString = ChuanglanSMS.batchSendVariable(url,account, pswd,msg,params,needstatus,extno);
			System.out.println(returnString);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void getReport() {
		...
		try {
			String returnString = ChuanglanSMS.getFlockStatusReport(url, account, key, count);
			System.out.println(returnString);
		} catch (Exception e) {
			e.printStackTrace();
		}
